...
function IconHamburger({ open }) {
  return (
    <div className="relative h-10 w-10">
      <span
        className={`absolute left-1/2 top-1/2 h-[2px] w-6 -translate-x-1/2 -translate-y-1/2 bg-white transition-transform duration-300 ${open ? "translate-y-0 rotate-45" : "-translate-y-2 rotate-0"
          }`}
      />
      <span
        className={`absolute left-1/2 top-1/2 h-[2px] w-6 -translate-x-1/2 -translate-y-1/2 bg-white transition-all duration-300 ${open ? "opacity-0" : "opacity-100"
          }`}
      />
      <span
        className={`absolute left-1/2 top-1/2 h-[2px] w-6 -translate-x-1/2 -translate-y-1/2 bg-white transition-transform duration-300 ${open ? "translate-y-0 -rotate-45" : "translate-y-2 rotate-0"
...
export default function Header({ navSolid, menuOpen, setMenuOpen, scrollToId, animationReady = false }) {
  ...
  return (
    <header
      ref={headerRef}
      className={`fixed left-0 top-0 z-50 w-full transition-all duration-300 ${navSolid ? "bg-black/55 backdrop-blur-md" : "bg-transparent"
        }`}
